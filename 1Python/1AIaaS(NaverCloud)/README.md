...
* 종류 : 회귀(Regression) 분류(Classification) 트리(Tree) 비지도(Unsupervised)
    * 회귀(regression) : 연속성 결과, 시퀀셜 결과, float 값. (ex. 고객 별 연체 확률 예측, 상품 판매량 예측)
    * 분류(classification) : 이산성 결과, 카테고리컬 결과, integer 값. (ex. 이중분류_라쿤인가?, 다중분류_어떤동물?)
...
* 스캐폴드 : 이미 만들어진 양식
* 설정값 = 맥락 = context
* 가상환경 : 독립적인 환경변수 구성의 환경. 라이브러리간 충돌을 방지한다.<br><br>