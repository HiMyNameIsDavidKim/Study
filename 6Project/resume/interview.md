...
* 문제점은 우리가 사용하는 PDF, 워드, 엑셀 같은 문서들이 조판용 구조이거나 테이블로 되어 있어 파싱 문제로 잘 작동하지 않습니다.
* 해결하기 위해 먼저 디텍션 하는 것이 필요합니다.
#### ToD에 대하여 설명하라
* ToD는 Task-oriented Dialogue로 특정 목적을 달성하기 위한 대화 시스템입니다. 예를 들어, 식당 예약 같은 구체적인 작업을 완료하기 위한 대화 태스크 입니다. 
#### ResNet의 핵심 개념을 설명하라
...
#### 지원동기
* 평소 (도메인)에 대한 확고한 목표를 가지고 있었고, (직무에서 하는일) 경험을 통해 (직무명)에 대한 확신이 생겼기 때문에 지원하게 되었습니다.
* (도메인 관련 경험)을 통해 (회사명) 입사에 대한 꿈을 가졌습니다.
* (직무 관련 경험)을 통해 (도메인)에 (직무)를 활용하는 목표에 대한 확신이 생겼습니다.
* 제가 가지고 있는 (도메인)에 대한 확고한 목표와 (직무 관련 경험)을 통해 쌓은 (직무에서 하는일) 역량을 발휘해 (회사명)과 함께 발전할 수 있다는 생각을 하여 지원하게 되었습니다.
#### 장단점
...
* (역량, 소프트 스킬)을 (구체적인 방법론)으로 (서비스명)에 활용하겠습니다.
...
#### 지원동기_DS
* (직무 관련 경험)
    * 또한 LGD에서 애플워치 자동화검사기의 알고리즘을 설계하는 과정에서 AI에 대한 관심이 크게 생겼고 AI 모델링 역량을 키우기 위해 대학원에 진학했습니다. 새로운 것을 배우고 도전하는 것에 거부감이 없을 뿐만 아니라 직무에 대한 강력한 목표와 성장 의지가 있었기 때문에 끈기있게 공부를 했습니다. 이때 다진 역량을 바탕으로 FCViT라는 직소퍼즐 모델을 설계하여 해당 task에서 SOTA를 달성했습니다.
#### 장단점_DS
* (장단점)
