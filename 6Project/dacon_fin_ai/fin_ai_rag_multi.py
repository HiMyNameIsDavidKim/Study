...
[Rule Enforcement]
1. Answer in Korean.
2. You searched website to solve the problem. Review the search results first.
3. Output must strictly adhere to the specified structure. Adding any additional explanations is not permitted.
            ...
            if filename[:2] in ['01']:
                ...
                for conversations_json in df['conversations']:
                    ...
                    if isinstance(conversations_list, list) and len(conversations_list) >= 2:
                        human_msg = conversations_list[0].get('value', '') if conversations_list[0].get(
                            'from') == 'human' else ''
                        gpt_msg = conversations_list[1].get('value', '') if conversations_list[1].get(
                            'from') == 'gpt' else ''
...
def generate_batch_answers(batch_messages):
    """배치로 여러 질문에 대한 답변 생성"""
    # 각 메시지를 채팅 템플릿으로 변환
    batch_texts = []
    ...
    with torch.no_grad():
        ...
        for i, (input_ids, output_ids) in enumerate(zip(model_inputs.input_ids, generated_ids)):
            ...
            batch_generated_ids.append(generated_part)
...
def generate_answer(messages):
    """단일 질문에 대한 답변 생성 (기존 호환성 유지)"""
    return generate_batch_answers([messages])[0]
...
def extract_final_answer(answer, question_type):
    ...
    if question_type == "multiple_choice":
        ...
        final_answer_pattern = r"(?:\d+\.)?\s*[*_]*\s*최종\s*답안\s*[*_]*\s*[:：]?\s*(\d+)"
...
def create_prompt_with_rag(question, question_type):
    ...
    for i, result in enumerate(search_results, 1):
        context_documents.append(f"[Search Results {i}]\n{result['document']}")
...
def process_test_questions():
    ...
    if TEST_SAMPLE_SIZE is not None:
        test_questions = test['Question'].head(TEST_SAMPLE_SIZE).tolist()
    ...
    # 배치 단위로 질문 처리
    total_batches = (len(test_questions) + BATCH_SIZE - 1) // BATCH_SIZE

    for batch_idx in tqdm(range(0, len(test_questions), BATCH_SIZE), desc="Batch Inference", total=total_batches):
        batch_questions = test_questions[batch_idx:batch_idx + BATCH_SIZE]
        batch_prompts = []
        batch_question_types = []

        # 배치 내 각 질문에 대한 프롬프트 생성
        for question in batch_questions:
            question_type = determine_question_type(question)
            prompt = create_prompt_with_rag(question, question_type)
            batch_prompts.append(prompt)
            batch_question_types.append(question_type)

        # 배치로 답변 생성
        try:
            batch_raw_answers = generate_batch_answers(batch_prompts)
            batch_final_answers = []

            # 각 답변 후처리
            for raw_answer, question_type in zip(batch_raw_answers, batch_question_types):
                final_answer = extract_final_answer(raw_answer, question_type)
                batch_final_answers.append(final_answer)

            batch_final_answers = [a for a in batch_final_answers if a is not None]
            if len(batch_final_answers) == len(batch_question_types):
                preds.extend(batch_final_answers)
            else:
                raise ValueError("답변 수가 질문 수와 일치하지 않습니다.")
            ...
            # 오류 발생 시 단일 처리로 대체
            for prompt, question_type in zip(batch_prompts, batch_question_types):
                try:
                    ...
                    print(f"단일 처리도 실패: {single_error}")
        ...
        # 테스트 모드일 때 중간 결과 출력
        if TEST_SAMPLE_SIZE is not None and len(preds) <= TEST_SAMPLE_SIZE:
            current_batch_size = len(batch_questions)
            print(f"\n--- 배치 {batch_idx // BATCH_SIZE + 1} 완료 ({current_batch_size}개 문항) ---")
            for i, (question, answer) in enumerate(zip(batch_questions, preds[-current_batch_size:])):
                print(f"문항 {len(preds) - current_batch_size + i + 1}: {answer}")
            print("-" * 50)
    ...
    print(f"배치당 평균 시간: {elapsed_time / total_batches:.2f}초")
