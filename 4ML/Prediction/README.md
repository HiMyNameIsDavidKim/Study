# Prediction(예측)

## `[examples]`
* [`뇌졸중 예측 Decision Tree`](https://github.com/HiMyNameIsDavidKim/ML_Example/blob/main/prediction/stroke.py)
* [`주가 분석 LSTM 앙상블`](https://github.com/HiMyNameIsDavidKim/ML_Example/blob/main/prediction/stock_lstm_ensemble.py)
* [`주가 분석 LSTM`](https://github.com/HiMyNameIsDavidKim/ML_Example/blob/main/prediction/stock_lstm.py)
* [`주가 분석 DNN 앙상블`](https://github.com/HiMyNameIsDavidKim/ML_Example/blob/main/prediction/stock_dnn_ensemble.py)
* [`주가 분석 DNN`](https://github.com/HiMyNameIsDavidKim/ML_Example/blob/main/prediction/stock_dnn.py)
* [`Iris 품종 분류 DNN`](https://github.com/HiMyNameIsDavidKim/ML_Example/blob/main/prediction/iris.py)
<br><br>
