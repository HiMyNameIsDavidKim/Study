...
### [클래스와 메서드]
* 클래스(class) : 객체를 만들어 내기 위한 설계도이자 틀. 연관 변수와 메서드의 집합체.
    * 파스칼의 계산기 근본 개념에서 가져옴.
    * 첫글자는 무조건 대문자
* 메서드(method) : 객체가 할 수 있는 동작들. 클래스 내부에 정의되어 있는 함수.
    * 데카르트의 방법서설 개념에서 가져옴.
    * 첫글자는 무조건 소문자
* 객체(object) : SW 세계에 구현할 대상. 클래스에 의해 선언된 모양대로 생성된 실체.
* 인스턴스(Instance) : 설계도를 바탕으로 SW 세계에 구현된 실체. 객체를 실체화 하면 그것을 인스턴스라고 부름.<br><br>
...
* 설계기법 분류 : 완전 탐색, 분할 정복, 탐욕, 동적 프로그래밍, 조합
    * 완전탐색 안에 Brute Force, 순열, 재귀, 비트마스크, BFS가 있다.
* 주제별 분류 : 탐색(선형검색, 이진검색), 정렬(버블정렬, 선택정렬, 삽입정렬), 그래프<br><br>

### [Brute Force Search(완전 탐색 중 한 방법)]
* 모든 경우를 다 시도해보는 방법.
...
## `[인사이트]`
* [파이썬 자습서](https://docs.oracle.com/javase/tutorial/java/index.html)
* [오라클의 자바 자습서](https://docs.python.org/ko/3/tutorial/index.html)
