...
* 주제 별 분류 : 탐색(선형탐색, 이진탐색, 해시탐색), 정렬(버블정렬, 선택정렬, 삽입정렬, 병합정렬, 힙정렬, 퀵정렬), 그래프&트리(BFS, 다익스트라, 벨만-포드, 플로이드-워셜)
<br><br>
...
* 구현 방법 : for문 + while문 으로 힙트리 구성, for문 + while문 으로 힙트리 구동. 부모와 자식의 위치를 판정하는 개념 적용.
* 시간 복잡도 : O(NlogN)
<br><br>

### [정렬_퀵정렬]
* 피벗 이라는 임의의 기준을 사용하며, 피벗보다 작은건 다 왼쪽 큰건 다 오른쪽으로 몰아넣고, 피벗을 옮기는 작업을 반복.
* 파이썬의 내장 함수 list.sort()는 이 알고리즘을 기본으로 한다.
* 구현 방법 : 피벗 개념, for문 + if문 3단계.
* 시간 복잡도 : O(NlogN)
<br><br>
...
### [다익스트라]
* 최단거리 테이블을 만들고, 출발 노드로 부터 가장 가까운 노드 선택, 비용 계산하여 테이블 갱신, 반복.
* 장점 : 구현이 쉽다.
* 단점 : 그리디 탐색이므로 느리다.
* 시간 복잡도 : O(ElogE)
    * E = 간선 수
