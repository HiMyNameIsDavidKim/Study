...
### [기타 특수 행렬]
* 단위행렬
    * 대각 요소의 값이 모두 1이고 나머지는 0인 행렬
    * 크기가 1이고 사이즈만 있다.
* 대각행렬
    * 대각 요소의 값이 모두 0이 아니고 나머지는 0인 행렬
    * 정사각행렬이 아니어도 대각행렬이 존재한다.
    * 무조건 i=j 요소만 살아있으면 된다.
    * 단위행렬과 유사하지만 크기가 1이 아니다.
* 삼각행렬
    * 대각 요소의 값과 위/아래가 모두 0이 아니고 아래/위는 모두 0인 행렬
    * 하삼각행렬: 아래가 모두 0이 아닌 경우
    * 상삼각행렬: 위가 모두 0이 아닌 경우
    * 해를 찾기 쉬운 특징이 있다.
* 대칭행렬
    * A^(T) = A
...
* 반대칭행렬
    * A^(T) = -A
    * 주대각선을 기준으로 요소가 거울상으로 절대값이 같고 부호가 반대다.
<br><br>
