...
* z-score : z = (x-모평균)/모시그마, 각 x가 평균에서 얼마나 떨어져 있는지 수치화.
    * z-score 고려 시 아웃라이어(이상치)가 제거되므로 ML에서는 특히 표준화가 더 낫다.<br><br>
...
### [손실함수]
* loss function, 예측값과 실제값에 대한 오차를 줄이는데 사용한다.
* cost function, error function과 같은 텀이다.
* 머신러닝은 통계학의 손실함수를 그대로 사용한다.
    * 회귀ML 모델의 손실함수는 MSE(Mean Square Error)가 best.
    * 분류ML 모델의 손실함수는 CEE(Cross Entropy Error)가 best.<br><br>
