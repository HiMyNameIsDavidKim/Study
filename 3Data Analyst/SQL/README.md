...
* Inconsistency : 비일관(하면 안돼), 데이터 간 상호 연관 관계에 대해 명확히 정의해야 한다.<br><br>
...
* 반정규화의 절차
    * 반정규화 대상조사(범위처리 빈도수, 범위, 통계성)
        * 자주 사용되는 데이터가 있는 경우.
        * 항상 일정한 범위만을 조회하는 경우.
        * 대량의 데이터를 불필요하게 넓은 범위로 처리하는 경우.
        * 통계 정보를 필요로 하는 경우.
        * 지나치게 많은 조인으로 데이터 조회가 어려운 경우.
    * 다른 방법유도 검토(뷰, 클러스터링, 인덱스 조정)
        * 뷰 : 가상의 테이블. 지나치게 많은 조인이 있을 때 사용.
        * 클러스터링 : 조회를 자주할 때 사용.
        * 파티셔닝 : 대량의 데이터는 부분적으로 분리.
        * 캐시 : 로직 구사 방법 변경.
* 반정규화 적용 방법
    * 테이블 : 테이블 병합(1대1, 1대M, 슈퍼&서브 타입), 테이블 분할(수직분할, 수평분할), 중복 추가, 통계 사용, 이력 추가, 부분 추가
    * 컬럼 : 중복 추가, 이력 추가, 계산된 컬럼 추가
    * 관계 : 중복 추가, 로우 체이닝, 로우 마이그레이션
    * PK : range 파티셔닝, list 파티셔닝, hash 파티셔닝<br><br>
    
### [슈퍼&서브 타입 모델]
* 데이터를 공통점과 차이점의 특징을 고려하여 표현하는 모델.
    * 슈퍼타입 : 공통부분만 빼서 생성한 엔터티
    * 서브타입 : 각 엔터티의 독립된 속성<br><br>

### [분산DB]
* 여러 곳으로 분산된 DB를 하나의 가상 시스템으로 사용할 수 있게 만든 DB
* 시스템으로는 묶여있으나, 네트워크를 통해 물리적으로는 분산되어 있다.
* 분산 DB가 만족해야 하는 투명성 6가지
...
* 외부키 : 다른 테이블의 기본키로 사용되는 키, 관계를 연결하는 컬럼.<br><br>