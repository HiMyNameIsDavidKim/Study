...
* 유니크한 갯수 세기
    * 해당 날짜에 유저가 몇명 방문 했는지 보기
    * 이때는 뷰수가 아니라 유저수 이므로 유니크 값을 봐야한다.
    * COUNTD() 사용해서 계산된 필드 만들기
* 날짜 단위 매개변수로 토글하기
    * 매개변수 만들기, 문자열, 목록형, 값을 (day, week, year)로 하기
    * 계산된 필드 만들기
    * 코드 작성
    * DATETRUNC([p_날짜단위], [Event Time], 'monday')
* 요일별 사용자 보기
    * 열 선반에 날짜 2개 넣기 -> 1 (더보기, 요일), 2(일)
    * 행 선반에 데이터 넣기
    * 분석 패널에서 평균라인 패널에 넣기
* 페이지 만들기
    * 인덱스 함수와 매개변수 활용 
    * 'index' 필드 만들기
    * 계산된 필드 만들기 -> 코드 작성: index()
    * 'p_페이지' 매개변수 만들기
    * 매개변수 만들기 -> 데이터 유형 정수, 목록 20, 40, 60, 80, 100
    * '페이지' 필드 만들기
    * 코드 작성
    * [p_페이지] >= [index]
    and [index] > [p_페이지] - 20
    * 페이지 필드를 필터에 넣고 참일 때로 선택
* 대시보드 레이아웃 너비 같게 하기
    * 하나의 컨테이너에 넣기 (ex. 가로 컨테이너)
    * 시트 하나 클릭 -> 회색 손잡이 더블클릭 -> 파란 손잡이 변경된다.
    * 아래 삼각형 -> 콘텐츠 균등 분할
* 대시보드 레이블 플로팅하게 바꾸기
    * 해당 레이블 클릭 -> 아래 삼각형 -> 부동
<br><br>
